...
static int setup_pins(){
    ...
    r+=hal_pin_port_new("trajectory.port",HAL_IN,&(port->Pin),comp_idx);

    //hal_signal_new("signal_filename",HAL_PORT);
    hal_link("trajectory.port","signal_filename");

    //! Input pins, type bit.
    enable = (bit_data_t*)hal_malloc(sizeof(bit_data_t));
    r+=hal_pin_bit_new("trajectory.enable",HAL_IN,&(enable->Pin),comp_idx);

    m3_in = (bit_data_t*)hal_malloc(sizeof(bit_data_t));
    r+=hal_pin_bit_new("trajectory.m3_in",HAL_IN,&(m3_in->Pin),comp_idx);
    m7_in = (bit_data_t*)hal_malloc(sizeof(bit_data_t));
    r+=hal_pin_bit_new("trajectory.m7_in",HAL_IN,&(m7_in->Pin),comp_idx);
    m8_in = (bit_data_t*)hal_malloc(sizeof(bit_data_t));
    r+=hal_pin_bit_new("trajectory.m8_in",HAL_IN,&(m8_in->Pin),comp_idx);

    //! Output pins, type bit.
    m3_out = (bit_data_t*)hal_malloc(sizeof(bit_data_t));
    r+=hal_pin_bit_new("trajectory.m3_out",HAL_OUT,&(m3_out->Pin),comp_idx);
    m7_out = (bit_data_t*)hal_malloc(sizeof(bit_data_t));
    r+=hal_pin_bit_new("trajectory.m7_out",HAL_OUT,&(m7_out->Pin),comp_idx);
    m8_out = (bit_data_t*)hal_malloc(sizeof(bit_data_t));
    r+=hal_pin_bit_new("trajectory.m8_out",HAL_OUT,&(m8_out->Pin),comp_idx);
    ...
    r+=hal_param_bit_new("trajectory.jog_x_plus",HAL_RW,&(jog_x_plus->Pin),comp_idx);
    jog_x_min = (param_bit_data_t*)hal_malloc(sizeof(param_bit_data_t));
    r+=hal_param_bit_new("trajectory.jog_x_min",HAL_RW,&(jog_x_min->Pin),comp_idx);

    jog_y_plus = (param_bit_data_t*)hal_malloc(sizeof(param_bit_data_t));
    r+=hal_param_bit_new("trajectory.jog_y_plus",HAL_RW,&(jog_y_plus->Pin),comp_idx);
    jog_y_min = (param_bit_data_t*)hal_malloc(sizeof(param_bit_data_t));
    r+=hal_param_bit_new("trajectory.jog_y_min",HAL_RW,&(jog_y_min->Pin),comp_idx);

    jog_z_plus = (param_bit_data_t*)hal_malloc(sizeof(param_bit_data_t));
    r+=hal_param_bit_new("trajectory.jog_z_plus",HAL_RW,&(jog_z_plus->Pin),comp_idx);
    jog_z_min = (param_bit_data_t*)hal_malloc(sizeof(param_bit_data_t));
    r+=hal_param_bit_new("trajectory.jog_z_min",HAL_RW,&(jog_z_min->Pin),comp_idx);

    jog_euler_x_plus = (param_bit_data_t*)hal_malloc(sizeof(param_bit_data_t));
    r+=hal_param_bit_new("trajectory.jog_euler_x_plus",HAL_RW,&(jog_euler_x_plus->Pin),comp_idx);
    jog_euler_x_min = (param_bit_data_t*)hal_malloc(sizeof(param_bit_data_t));
    r+=hal_param_bit_new("trajectory.jog_euler_x_min",HAL_RW,&(jog_euler_x_min->Pin),comp_idx);

    jog_euler_y_plus = (param_bit_data_t*)hal_malloc(sizeof(param_bit_data_t));
    r+=hal_param_bit_new("trajectory.jog_euler_y_plus",HAL_RW,&(jog_euler_y_plus->Pin),comp_idx);
    jog_euler_y_min = (param_bit_data_t*)hal_malloc(sizeof(param_bit_data_t));
    r+=hal_param_bit_new("trajectory.jog_euler_y_min",HAL_RW,&(jog_euler_y_min->Pin),comp_idx);

    jog_euler_z_plus = (param_bit_data_t*)hal_malloc(sizeof(param_bit_data_t));
    r+=hal_param_bit_new("trajectory.jog_euler_z_plus",HAL_RW,&(jog_euler_z_plus->Pin),comp_idx);
    jog_euler_z_min = (param_bit_data_t*)hal_malloc(sizeof(param_bit_data_t));
    r+=hal_param_bit_new("trajectory.jog_euler_z_min",HAL_RW,&(jog_euler_z_min->Pin),comp_idx);

    //! Names like start, stop, will give error in hal.
    start_ = (param_bit_data_t*)hal_malloc(sizeof(param_bit_data_t));
    r+=hal_param_bit_new("trajectory.start",HAL_RW,&(start_->Pin),comp_idx);
    ...
    r+=hal_param_float_new("trajectory.start_from_gcodeblock",HAL_RW,&(start_from_gcodeblock->Pin),comp_idx);

    //! Input pins, type float.
    max_velocity = (float_data_t*)hal_malloc(sizeof(float_data_t));
    r+=hal_pin_float_new("trajectory.max_velocity",HAL_IN,&(max_velocity->Pin),comp_idx);

    max_acceleration = (float_data_t*)hal_malloc(sizeof(float_data_t));
    r+=hal_pin_float_new("trajectory.max_acceleration",HAL_IN,&(max_acceleration->Pin),comp_idx);

    max_jerk = (float_data_t*)hal_malloc(sizeof(float_data_t));
    r+=hal_pin_float_new("trajectory.max_jerk",HAL_IN,&(max_jerk->Pin),comp_idx);

    velocity_override = (float_data_t*)hal_malloc(sizeof(float_data_t));
    r+=hal_pin_float_new("trajectory.velocity_override",HAL_IN,&(velocity_override->Pin),comp_idx);

    power_rpm_in = (float_data_t*)hal_malloc(sizeof(float_data_t));
    r+=hal_pin_float_new("trajectory.power_rpm_in",HAL_IN,&(power_rpm_in->Pin),comp_idx);

    tcp_offset_x = (float_data_t*)hal_malloc(sizeof(float_data_t));
    r+=hal_pin_float_new("trajectory.tcp_offset_x",HAL_IN,&(tcp_offset_x->Pin),comp_idx);

    tcp_offset_y = (float_data_t*)hal_malloc(sizeof(float_data_t));
    r+=hal_pin_float_new("trajectory.tcp_offset_y",HAL_IN,&(tcp_offset_y->Pin),comp_idx);

    tcp_offset_z = (float_data_t*)hal_malloc(sizeof(float_data_t));
    r+=hal_pin_float_new("trajectory.tcp_offset_z",HAL_IN,&(tcp_offset_z->Pin),comp_idx);

    //! Output pins, type float.
    tcp_x = (float_data_t*)hal_malloc(sizeof(float_data_t));
    r+=hal_pin_float_new("trajectory.tcp_x",HAL_OUT,&(tcp_x->Pin),comp_idx);

    tcp_y = (float_data_t*)hal_malloc(sizeof(float_data_t));
    r+=hal_pin_float_new("trajectory.tcp_y",HAL_OUT,&(tcp_y->Pin),comp_idx);

    tcp_z = (float_data_t*)hal_malloc(sizeof(float_data_t));
    r+=hal_pin_float_new("trajectory.tcp_z",HAL_OUT,&(tcp_z->Pin),comp_idx);

    tcp_euler_x = (float_data_t*)hal_malloc(sizeof(float_data_t));
    r+=hal_pin_float_new("trajectory.tcp_euler_x",HAL_OUT,&(tcp_euler_x->Pin),comp_idx);

    tcp_euler_y = (float_data_t*)hal_malloc(sizeof(float_data_t));
    r+=hal_pin_float_new("trajectory.tcp_euler_y",HAL_OUT,&(tcp_euler_y->Pin),comp_idx);

    tcp_euler_z = (float_data_t*)hal_malloc(sizeof(float_data_t));
    r+=hal_pin_float_new("trajectory.tcp_euler_z",HAL_OUT,&(tcp_euler_z->Pin),comp_idx);


    current_position = (float_data_t*)hal_malloc(sizeof(float_data_t));
    r+=hal_pin_float_new("trajectory.current_position",HAL_OUT,&(current_position->Pin),comp_idx);

    current_velocity = (float_data_t*)hal_malloc(sizeof(float_data_t));
    r+=hal_pin_float_new("trajectory.current_velocity",HAL_OUT,&(current_velocity->Pin),comp_idx);

    current_acceleration = (float_data_t*)hal_malloc(sizeof(float_data_t));
    r+=hal_pin_float_new("trajectory.current_acceleration",HAL_OUT,&(current_acceleration->Pin),comp_idx);

    power_rpm_out = (float_data_t*)hal_malloc(sizeof(float_data_t));
    r+=hal_pin_float_new("trajectory.power_rpm_out",HAL_OUT,&(power_rpm_out->Pin),comp_idx);
